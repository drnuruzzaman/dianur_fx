...
import { GRADE_CLASS, GRADE_TEXT, gradeFor } from '../chart/graded.js';
...
export class RulePanel {
  ...
  render() {
    ...
    /* THE FORWARD TEST'S GRADE WINS WHEN IT HAS ONE, because it is a record of
       the rule ACTUALLY RUNNING here. `rec` above comes from runs/, and every
       record there was produced by the flat 20/10 Donchian -- so on a
       horizon-matched frame it correctly answers `unmeasured`, and the panel
       correctly printed NOT MEASURED while the Signal Board printed `marginal`
       with numbers for the same cell. Both true, and together they read as a
       contradiction that sends the reader off to check elsewhere.

       configs/alerts.json is the pre-registered grade for the horizon-matched
       channel, which is what this chart runs. Preferring it removes the
       disagreement without inventing anything: the grade and the expectation in
       the tooltip were both measured, and neither is computed here. */
    const graded = gradeFor(this.symbol, rawTf2);
    const useGrade = graded && GRADE_TEXT[graded.grade]
                     && rec.verdict === 'unmeasured';
    const cellV = el('span', { class: 'rp-cell-v' },
      useGrade ? GRADE_TEXT[graded.grade]
               : (VERDICT_TEXT[rec.verdict] || rec.verdict));
    if (useGrade) {
      cellV.classList.add(GRADE_CLASS[graded.grade] || '');
      /* The pre-registered sentence, verbatim from the file -- it already
         carries the era numbers and the date the expectation was frozen. */
      if (graded.note) cellV.title = graded.note;
    }
    r.appendChild(el('div', { class: 'rp-cell' },
      el('span', { class: 'rp-cell-k' }, 'this cell'), cellV));
