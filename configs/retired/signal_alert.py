...
from tools.event_alert import BNE                              # noqa: E402
...
def read_held(timeout=30):
    """{symbol: 'long'|'short'} for whatever the terminal is holding.

    ONE CALL FOR THE WHOLE RUN. The exit pass needs to know which symbols have
    a position, and asking /signal twice for every cell would double the load on
    a bridge that is already fetching a thousand bars per fast cell. A failure
    here is NOT fatal: it costs the exit alerts for this run and leaves every
    entry alert intact, which is the right way round.
    """
    try:
        with urllib.request.urlopen(BRIDGE + '/positions', timeout=timeout) as r:
            payload = json.loads(r.read().decode())
    except Exception as err:                                   # noqa: BLE001
        print('positions unavailable, exit alerts skipped this run (%s)' % err,
              file=sys.stderr)
        return {}
    legs = payload if isinstance(payload, list) else (payload.get('positions') or [])
    net = {}
    for leg in legs:
        sym = leg.get('symbol') or ''
        vol = float(leg.get('volume') or 0)
        net[sym] = net.get(sym, 0.0) + (
            vol if str(leg.get('side')).lower() == 'buy' else -vol)
    return {k: ('long' if v > 0 else 'short')
            for k, v in net.items() if abs(v) > 1e-9}


def poll(symbol, tf, strategy=None, timeout=90, position=None):
    ...
    `position` OVERRIDES WHAT THE BRIDGE READS OFF THE TERMINAL, and the caller
    passes 'flat' for the entry pass. Left alone, /signal reads the live
    position -- correct for an order ticket, and the reason this alerter went
    weeks without emitting a single entry. Eleven watched cells share two
    account positions, so while any gold was open every gold cell believed it
    was already in and answered `hold` to a breakout it should have called.
    See the ENTRY PASS / EXIT PASS comment at the call site.
    """
...
def main():
    ...
    # WHERE SIGNALS GO. A list in configs/alerts.json, one entry per chat or
    # WhatsApp number, filtered to the ones that asked for `signals` -- so a
    # group can take the macro calendar and not every 5m entry. Resolved ONCE
    # per run, not per cell: a list that changed mid-run would send two cells'
    # signals to two different sets of chats.
    # Reads configs/alerts.json itself -- `load_config()` above returns only
    # the signals half, and handing this a slice that does not contain
    # `destinations` would silently fall back to the TELEGRAM_CHAT_ID list and
    # ignore everything configured in the UI.
    dests = notify.destinations(kind='signals')
    ...
    for sym, tf, grade, strat in cells(cfg):
        try:
            # THE ENTRY PASS, asked flat: does the rule want a trade on this
            # cell, whatever the account happens to hold.
            sig = poll(sym, tf, strat, position='flat')
            # THE EXIT PASS, and only for a symbol actually held. An exit is a
            # statement about a position, so it cannot come from the flat pass,
            # and asking every cell twice would double the bridge load for the
            # nine cells whose symbol is flat. One /positions read covers all.
            if (sig.get('action') or '').lower() == 'hold' and held.get(sym):
                alt = poll(sym, tf, strat, position=held[sym])
                if (alt.get('action') or '').lower() == 'exit':
                    sig = alt
        ...
        if not token or not dests:
            print('WOULD SEND:\n%s\n' % text)
            print('no route for signals: %s'
                  % ('TELEGRAM_BOT_TOKEN missing from configs/secrets.env'
                     if not token else
                     'no enabled destination wants signals -- '
                     'Settings -> Destinations'), file=sys.stderr)
            return 2
        # A PARTIAL SEND COUNTS AS SENT, and the dedupe ledger below is why: a
        # destination that was down misses this message rather than every
        # destination getting it again next poll. A duplicate entry signal can
        # be acted on twice; a missing copy of one cannot.
        res = notify.deliver('signals', text, dests=dests)
        if not res['sent']:
            print('%s %s: send failed -- %s'
                  % (sym, tf, '; '.join(r.get('error', '?')
                                        for r in res['results'])),
                  file=sys.stderr)
            failed += 1
            continue
        if res['failed']:
            print('%s %s: sent to %d of %d destinations'
                  % (sym, tf, res['sent'], res['total']), file=sys.stderr)
